...
function play() {
	...
	function setDraggable() {
		if (playing == 1) {
			for (let i = 0; i < 5; i++) {
				...
				zonePlayerOne.style.background =
					"url('assets/img/palet1.png') no-repeat left top";
			...
			for (let i = 5; i < 10; i++) {
				image[i].setAttribute("draggable", "true");
				zonePlayerTwo.style.background =
					"url('assets/img/palet2.png') no-repeat right top";
	...
	function drop(ev) {
		...
		for (let i = 0; i < cardPower.length; i++) {
			tableCard[ev.target.id.substr(2, 1) - 1][i] =
				cardPower[i].innerHTML;
		}
		...
		switch (ev.target.id.substr(2, 1)) {
			case "1":
				console.log(ev.target.id.substr(2, 1));
				if (
					parseInt(tableCard[ev.target.id.substr(2, 1) - 1][2]) >
						tableCard[1][1] &&
					tableCard[1][1] > -1
				) {
					if (idCard[0].id.substr(4, 2) < 6) {
				...
				if (
					parseInt(tableCard[ev.target.id.substr(2, 1) - 1][3]) >
						tableCard[3][0] &&
					tableCard[3][0] > -1
				) {
					if (idCard[0].id.substr(4, 2) < 6) {
			...
			case "2":
				console.log(ev.target.id.substr(2, 1));
				if (
					parseInt(tableCard[ev.target.id.substr(2, 1) - 1][1]) >
						tableCard[0][2] &&
					tableCard[0][2] > -1
				) {
					if (idCard[0].id.substr(4, 2) < 6) {
			...
			case "4":
				console.log(ev.target.id.substr(2, 1));
				if (
					parseInt(tableCard[ev.target.id.substr(2, 1) - 1][0]) >
						tableCard[0][3] &&
					tableCard[0][3] > -1
				) {
					if (idCard[0].id.substr(4, 2) < 6) {
		...
		if (playedCard == 9) {
			...
			scoreTab.innerHTML =
				"<p>" + win + "</p><p><a href='index.php'>Retour</a></p>";
		}
	}
	...
	for (let i = 0; i < cartes.length; i++) {
		cartes[i].addEventListener("dragstart", function(event) {
			drag(event);
		});
	}
